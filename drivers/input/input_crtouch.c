...
#ifdef CONFIG_INPUT_CRTOUCH_INTERRUPT
static void crtouch_irq_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
...
static int crtouch_init(const struct device *dev)
{
	...
	gpio_init_callback(&data->int_gpio_cb, crtouch_irq_handler, BIT(config->int_gpio.pin));
...
#define CRTOUCH_INIT(inst)                                                                         \
	...
	static struct crtouch_data crtouch_data_##inst = {};                                       \
                                                                                                   \
	DEVICE_DT_DEFINE(DT_DRV_INST(inst), &crtouch_init, NULL, &crtouch_data_##inst,             \
			 &crtouch_config_##inst, POST_KERNEL, CONFIG_INPUT_INIT_PRIORITY, NULL);
